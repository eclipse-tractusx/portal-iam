...
const sanitizeNames = (string) => string
    .split('\n')
    .map(line => line.match(/^\s+"name": "/)
        ? `"name": "${line.trim().substring(9, line.trim().length - 2).replace('"', "'")}",`
        : line
    )
    .join('\n')
...
window.onload = () => {
    ...
    const providers = [...document.getElementById('providers').childNodes].map(n => n.data).join('')
